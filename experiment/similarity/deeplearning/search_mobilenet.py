...
def load_search_database(split='train'):
    ...
    # 04b_mobilenet_extract.py에서 저장한 경로
    feature_dir = Path(config.FEATURE_SAVE_DIR) / "mobilenetv3" / config.SEED_DIR
    ...
    if not all_db_filenames:
        ...
        print(f"  '04b_mobilenet_extract.py'를 먼저 실행하세요.")
        sys.exit(1)
...
def extract_query_features(model, img_path):
    ...
    try:
        # MobileNetV3 이미지 크기 사용
        img = load_img(img_path, target_size=config.IMG_SIZE_MOBILENET)
        img_array = img_to_array(img)
        img_batch = np.expand_dims(img_array, axis=0)
        
        # MobileNetV3 전처리 함수 사용
...
def calculate_metrics(
    ...
    # (search_efficientnet.py와 동일한 코드)
