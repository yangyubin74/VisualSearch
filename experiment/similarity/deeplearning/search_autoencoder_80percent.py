...
# --- 공통 모듈 임포트 AutiEncoder은 정성적 평가 수행 ---
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))
try:
    import config
    from common_utility import measure_process_time
except ImportError:
    print("❌ 오류: config.py 또는 common_utility.py를 찾을 수 없습니다.")
    sys.exit(1)

# ============================================================================
# --- 1. 환경설정 ---
# ============================================================================
# [수정] 정성적 평가를 위해 Top-10 설정
TOP_K = 10 
TARGET_IMAGE_PATH = None

# [제거] 품질 지표 관련 변수 모두 제거
...
def load_search_database_ae(split='train') -> Tuple[np.ndarray, List[str]]:
    """Autoencoder 'train' 스플릿(80%)의 특징 벡터와 파일명 리스트를 로드합니다."""
...
def extract_query_features_ae(model: Model, img_path: str) -> np.ndarray:
    """단일 쿼리 이미지의 Autoencoder 특징 벡터를 추출합니다."""
    try:
        # 128x128 크기 사용
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        ...
        print("\n========> 사용법: python 09_search_autoencoder_80_ranking.py [이미지명]")
        print("  [이미지명]: 확장자(.png)를 제외한 타겟 이미지 파일명")
        print("\n  예시: python 09_search_autoencoder_80_ranking.py my_test_shirt_01")
        sys.exit(1)
